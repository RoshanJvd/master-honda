...
const InvoiceModal: React.FC<InvoiceModalProps> = ({ isOpen, onClose, data }) => {
  ...
  const triggerPrint = (e: React.MouseEvent) => {
    ...
    // Slight delay to ensure UI updates before print dialog
    setTimeout(() => {
      window.print();
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 overflow-y-auto">
          ...
          >
            ...
            <div className="p-4 border-b border-gray-100 flex items-center justify-between print:hidden bg-white">
            ...
            {/* Receipt Content (This is what actually gets printed) */}
            <div className="p-6 bg-white thermal-invoice">
              
              {/* Top Header - Centered */}
              <div className="text-center mb-6">
                <h1 className="text-[20px] font-black text-black tracking-tight leading-none mb-1">MASTER HONDA</h1>
              ...
              {/* Transaction Info Box - Dashed Borders */}
              <div className="border border-dashed border-black p-3 mb-6 space-y-1.5">
                ...
                <div className="border-t border-dashed border-black mt-2 pt-2">
                    <div className="flex justify-between items-start text-[10px] font-bold text-black">
                      <span className="uppercase">CUSTOMER:</span>
                      <span className="text-right flex-1 break-words ml-2">{data.customerName.toUpperCase()}</span>
                    </div>
                    <div className="flex justify-between items-center text-[10px] font-bold text-black mt-1">
                      <span className="uppercase">BIKE UNIT:</span>
                      <span className="text-right flex-1 break-words ml-2">{data.bikeModel.toUpperCase()}</span>
                    </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                <div className="border-b border-black pb-1 mb-2">
                  <div className="flex justify-between text-[9px] font-black uppercase tracking-wider">
                    ...
                    <span className="w-20 text-right">AMT (RS)</span>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-start gap-2">
                        <div className="flex-1">
                          <p className="font-bold text-black text-[11px] leading-tight break-words">{data.serviceType.toUpperCase()}</p>
                        </div>
                        <span className="font-bold text-black text-[11px] w-20 text-right">{data.servicePrice.toLocaleString()}</span>
                      </div>
                      
                      {data.additionalServices?.map((s, idx) => (
                        <div key={`s-${idx}`} className="flex justify-between items-start gap-2">
                          <div className="flex-1">
                            <p className="font-bold text-black text-[11px] leading-tight break-words">{s.name.toUpperCase()}</p>
                          </div>
                          <span className="font-bold text-black text-[11px] w-20 text-right">{s.price.toLocaleString()}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {(isService ? data.partsUsed : data.items)?.map((item, idx) => (
                    <div key={`p-${idx}`} className="flex justify-between items-start gap-2">
                      <div className="flex-1">
                        <p className="font-bold text-black text-[11px] leading-tight break-words">{item.partName.toUpperCase()}</p>
                        <p className="text-[9px] text-black font-medium mt-0.5 italic">QTY: {item.quantity} x {item.price.toLocaleString()}</p>
                      </div>
                      <span className="font-bold text-black text-[11px] w-20 text-right">{(item.price * item.quantity).toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              </div>

              {/* Grand Total Box - Prominent Dashed Border */}
              <div className="border border-dashed border-black p-4 mb-8">
                <div className="flex justify-between items-center">
                  <span className="text-[12px] font-black uppercase tracking-tighter">GRAND TOTAL</span>
                  <span className="text-[22px] font-black text-black">Rs. {calculateTotal().toLocaleString()}</span>
                </div>
              </div>
              ...
              <div className="text-center space-y-4">
                <div className="space-y-1">
                  ...
                  <p className="text-[9px] text-black italic font-medium opacity-80">"The Power of Dreams"</p>
                </div>
                
                <div className="pt-6 flex flex-col items-center">
                  <span className="text-[9px] font-black text-black uppercase tracking-widest mb-1.5">AUTHORIZED SIGNATURE</span>
                  <div className="w-40 border-b border-dashed border-black"></div>
                </div>
                
                <div className="pt-4 opacity-50">
                  <p className="text-[6px] text-black font-mono leading-none mb-1">**********************************</p>
